defmodule Battleship.Board do
  ...
  """
  @spec place_ships(board, [Battleship.Game.ship]) ::
    {:ok, board} | {:error, term}
  def place_ships(board, ships) when is_list(ships) do
    if valid_fleet_arrangement?(board, ships) do
      ...
      new_grid = Enum.reduce(coordinates, board.grid, fn(coordinate, grid) ->
  ...
  def status(board) do
    status_grid = Grid.map(board.grid, fn(cell) ->
      if cell.shot? do
  ...
  defp valid_fleet_arrangement?(board, ships) do
    ships_on_board?(board, ships) &&
      non_overlapping_ships?(board, ships) &&
